...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/home4/jiaxin/linwang/exp/CLIP-1219-multi-xiyouji-v4-ti-continue-xiyouji-v1-both-noproj",
    ...
    parser.add_argument("--train_stage", default=None,
        choices=["do_ti", "load_ti_do_both"],
        required=False)
    parser.add_argument("--learnable_property", choices=["object", "style"])
    parser.add_argument("--pretrained_ti_path", default=None, help="Needed when load ti")
    parser.add_argument("--placeholder_tokens", default=None)
    parser.add_argument("--initializer_tokens", default=None)
...
def main(args):
    ...
    if args.gradient_checkpointing:
        ...
        # unet.enable_gradient_checkpointing() # *6
    ...
    clip_big_model = CLIPModel.from_pretrained("laion/CLIP-ViT-bigG-14-laion2B-39B-b160k", ignore_mismatched_sizes=True)
    clip_big_processor = AutoProcessor.from_pretrained("laion/CLIP-ViT-bigG-14-laion2B-39B-b160k")

    clip_big_model.to(accelerator.device)
        ...
        for n, p in model.named_parameters():
            if "text_projection" in n:
                if args.train_stage in {'do_ti'}:
                    ...
                    continue  # TODO: 是否保留 text projection
            if p.requires_grad:
                params.append(p)
    ...
    def preprocess_train(examples):
        ...
        for i in examples[image_column]:
            ...
            image_big_inputs = clip_big_processor(images=image, return_tensors="pt")
            all_big_clip_images.append(image_big_inputs.pixel_values[0])
        # image aug
        original_sizes = []
        all_images = []
        crop_top_lefts = []
        for image in images:
            ...
            image = train_transforms(image) # image aug torch.Size([3, 1024, 1024])
            all_images.append(image)# len is batch size

        examples["original_sizes"] = original_sizes
        examples["crop_top_lefts"] = crop_top_lefts
        examples["pixel_values"] = all_clip_images
        examples["big_pixel_values"] = all_big_clip_images
    ...
    def collate_fn(examples):
        ...
        big_pixel_values = torch.stack([example["big_pixel_values"] for example in examples])
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(text_encoder_one), accelerator.accumulate(text_encoder_two):
                ...
                image_inputs = batch["pixel_values"]
                ...
                image_big_inputs = batch["big_pixel_values"]
                image_big_features = clip_big_model.get_image_features(image_big_inputs.to(clip_big_model.device))
                text_big_features = clip_big_model.text_projection(pooled_prompt_embeds.to(clip_big_model.device))
                
                with autocast():
                    logit_scale = clip_model.logit_scale.exp()
                    big_logit_scale = clip_big_model.logit_scale.exp()
                    loss = clip_loss(image_features, text_features, logit_scale) + clip_loss(image_big_features, text_big_features, big_logit_scale)
    ...
    if accelerator.is_main_process:
        ...
        if args.train_stage == 'do_ti' or args.train_stage == 'load_ti_do_both':
